...
# How often (seconds) the live dashboard is actually allowed to send an
# edit to Telegram. Progress is tracked continuously in memory either
# way; this just throttles how often that gets pushed to the message,
# so a fast job doesn't spam edits or hit message-edit rate limits.
STATUS_UPDATE_SECONDS = config.get("status_update_seconds", 5)

# If the user taps "Cancel" but never confirms, auto-dismiss the confirm
# prompt after this long so the job doesn't sit there forever waiting.
CANCEL_CONFIRM_TIMEOUT_SECONDS = config.get("cancel_confirm_timeout_seconds", 20)
...
def render_dashboard(job: "ForwardJob", state: str) -> str:
    ...
    if state == "cancelling":
        if job.pending_count > 0:
            lines.append(
                f"Stopping — letting {job.pending_count} in-flight batch"
                f"{'es' if job.pending_count != 1 else ''} finish, no new work is being started."
            )
        else:
            lines.append("Stopping — wrapping up now.")
        lines.append("")
    ...
    if state == "cancelled":
        lines.append(f"🛑 Stopped early — {remaining:,} messages in the range were never checked.")
        lines.append("")
...
class ForwardJob:
    ...
    def _check_cancel_confirm_timeout(self):
        """If the user tapped Cancel but never confirmed, drop the confirm
        prompt after CANCEL_CONFIRM_TIMEOUT_SECONDS so it doesn't get stuck."""
        if self.awaiting_cancel_confirm and (
            time.time() - self.cancel_ask_ts > CANCEL_CONFIRM_TIMEOUT_SECONDS
        ):
            self.awaiting_cancel_confirm = False

    async def _update_status(self, state: str, force: bool = False):
        """Refresh the dashboard. Throttled to STATUS_UPDATE_SECONDS unless
        force=True (used for things the user should see immediately, like
        a button press or the job finishing)."""
        self.state = state
        self._check_cancel_confirm_timeout()

        now = time.time()
        if not force and (now - self._last_update_ts) < STATUS_UPDATE_SECONDS:
            return
    ...
    async def run(self):
        ...
        for chunk_start in range(0, len(all_ids), FETCH_CHUNK_SIZE):
            ...
            for msg_id, msg in zip(id_chunk, msgs):
                ...
                self.processed += 1
                await self._update_status("Forwarding")  # no-ops unless 5s have passed
                ...
                if len(pending_batch) >= MAX_PER_WORKER:
                    ...
                    self.pending_count = len(pending_tasks)
                    if not ok:
                        stopped = True
                        break
        ...
        if self.cancel_requested:
            # Graceful stop: no new batches are dispatched (see checks
            # above), but whatever's already mid-flight is allowed to
            # finish rather than being abandoned half-sent.
            self.pending_count = len([t for t in pending_tasks if not t.done()])
            await self._update_status("cancelling", force=True)

        if pending_tasks:
            while any(not t.done() for t in pending_tasks):
                self.pending_count = len([t for t in pending_tasks if not t.done()])
                await self._update_status("cancelling" if self.cancel_requested else "Forwarding")
        ...
        if self.cancel_requested:
            await self._update_status("cancelled", force=True)
        else:
            await self._update_status("done", force=True)
...
async def callback_handler(event):
    ...
    # Button presses always force an immediate refresh -- the 5s throttle
    # is for automatic ticking only, not for direct user actions.
    if data == b"pause":
        job.paused = True
        await event.answer("Paused")
        await job._update_status(job.state, force=True)

    elif data == b"resume":
        job.paused = False
        await event.answer("Resumed")
        await job._update_status("Forwarding", force=True)

    elif data == b"cancel_ask":
        job.awaiting_cancel_confirm = True
        job.cancel_ask_ts = time.time()
        await event.answer(f"Confirm within {CANCEL_CONFIRM_TIMEOUT_SECONDS}s or this is dismissed.")
        await job._update_status(job.state, force=True)
        ...
        job.paused = False
        await event.answer("Cancelling -- finishing in-flight batches...")
        await job._update_status("cancelling", force=True)
